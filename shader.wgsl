...
fn fragment(input : FragmentInput) -> FragmentOutput {
    ...
    let N = normalize(input.normal);        //normalizirana normala
    let L = normalize(light.position - input.position);      //normaliziran vektor luči
    
    let lambert = max(dot(N, L), 0);        //lambertov osvetlitveni faktor

    let ambientFactor = vec4(vec3(light.ambient), 1);        //ambientni faktor
